...
@Autowired
 ...
 public ResponseEntity<?> login(@RequestBody User user,HttpSession session )
 {
	 try {
	 if
	 (userService.validateUser(user.getUserId(), user.getUserPassword())) {
	 session.setAttribute("Logged in", user.getUserId());
	 return new ResponseEntity<String>("Successfully logged In",HttpStatus.OK);
	 }
	 else {
		 return new ResponseEntity<String>("Successfully logged In",HttpStatus.UNAUTHORIZED);
 ...
 if(session.getAttribute("loggedInUserId")!=null)
	 {
		 session.invalidate();
		 return new ResponseEntity<String>("LogoutSuccessfull",HttpStatus.OK);
	 }
	else {
